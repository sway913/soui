...
`.github/workflows/gui-sanitizer-pilot.yml` 的 Linux Sanitizer 任务使用 Clang 为项目和 `fun_test` 编译、链接 ASan+UBSan，并在 Xvfb 下运行 SOUI 的 8 个核心单元、集成及无窗口 E2E 用例。Scintilla 的跨平台构建包含 Windows COM 实现 `HanjaDic.cxx`；其接口在非 Windows 平台缺少 RTTI 定义，因此只对该源文件关闭 UBSan 的 `vptr` 检查。`swinx` 子模块的 `sysobjs.cpp` 在共享内存中建立的句柄表存在 8 字节对齐问题，会在程序启动时触发 UBSan；试点通过 `.github/sanitizer-ignorelist.txt` 只排除该文件的 `alignment` 检查。其余源文件仍使用 ASan+UBSan 插桩，这两处已知例外不代表相关缺陷已修复。该任务目前是独立试点，不作为 `master` 的合并必需检查；待实跑结果稳定并处理发现的问题后，再决定是否扩展覆盖或纳入门禁。
